type AnyObject = {_:string[];} & {
	[key:string|number|symbol]:string[]|string;
};

export = class {
	static parse(args:string[]):AnyObject {
		const return_value:AnyObject = { _:[] };
		...
		while(_args.length > 0) {
			...
			let key:string, curr_val:string|string[], next_val:string;
